...
def order_history_to_df(reader: ZipArchiveReader, errors: Counter, validation) -> pd.DataFrame:
    ...
    -------
    pd.DataFrame
        Columns: ``Order date``, ``Total order price``, ``Order status``, ``Product name``, ``Variation name``, ``Quantity``
        Empty DataFrame when the data is absent or parsing fails.
    ...
    Table documentation::
        {
          ...
          "columns": {
            ...
            "Total order price": "Price for the full order.",
    ...
    Table config::
        {
          ...
          "headers": {
            "Order date": {"en": "Order date", "nl": "Orderdatum en tijd"},
            ...
            "Variation name": {"en": "Variation name", "nl": "Variatienaam"},
    ...
    try:
        ...
        out = pd.DataFrame(rows, columns=["Order date", "Total order price", "Order status", "Product name", "Variation name", "Quantity"])  # pyright: ignore
    except Exception as e:
        logger.error("Exception caught: %s", e)
        errors[type(e).__name__] += 1
        return out
